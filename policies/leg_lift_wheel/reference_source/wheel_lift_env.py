"""Wheeled adaptation of leg-lift-upright-pose (0a2b6cf).
Retains its PPO task/reward backbone. Learns eight proximal position targets;
hubs hold zero during lift-only training and are absent from actor observations.
This is a new training contract; original leg-lift checkpoints are incompatible.
"""
...
from workspace import wheel_lift_config as configs
...
class PupperWheelLiftEnv(PipelineEnv):
    ...
    def __init__(self, config: config_dict.ConfigDict, model_path: Optional[str] = None):
        ...
        sys = sys.replace(
            actuator_gainprm=sys.actuator_gainprm.at[:, 0].set(jp.array(config.position_control_kp)),
            actuator_biasprm=sys.actuator_biasprm.at[:, 1]
            .set(-jp.array(config.position_control_kp))
            .at[:, 2]
            .set(-jp.array(config.dof_damping)),
        )
        ...
        sys.mj_model.actuator_biasprm[:, 1] = -np.array(config.position_control_kp)
        sys.mj_model.actuator_biasprm[:, 2] = -np.array(config.dof_damping)
        ...
        self._stand_height = float(self._init_q[2])
        self._floor = sys.mj_model.geom("floor").id
        self._wheels = np.array([sys.mj_model.geom("leg_"+leg+"_3_wheel_collision").id for leg in configs.FOOT_ROW_BY_LEG])

        # ---- observation sizing ----
        # [angular(3), gravity(3), command(5), proximal position(8), previous proximal action(8)]
        self._single_obs_dim = 3 + 3 + self._num_commands + 8 + 8
    ...
    def wheel_bottoms(self, ps):
        axis_z = ps.geom_xmat[self._wheels, 2, 2]
        return ps.geom_xpos[self._wheels, 2] - .048*jp.sqrt(jp.maximum(1-axis_z**2, 0)) - .01675*jp.abs(axis_z)

    def wheel_loads(self, ps):
        # Native MJX pyramidal condim=3 contacts: four friction-pyramid rows.
        contact = ps.contact
        address = contact.efc_address
        indices = jp.clip(address[:,None]+jp.arange(4),0,ps._impl.efc_force.shape[0]-1)
        force = jp.sum(ps._impl.efc_force[indices],axis=1)
        valid = (address>=0)&(contact.dist<=0)&jp.any(contact.geom==self._floor,axis=1)
        match = jp.any(contact.geom[:,:,None]==jp.array(self._wheels)[None,None,:],axis=1)
        return jp.sum(jp.where(valid[:,None]&match,jp.maximum(force[:,None],0),0),axis=0)
    ...
    def step(self, state: State, action: jax.Array) -> State:
        action = jp.zeros(12).at[configs.PROXIMAL].set(action)
        ...
        joint_angles = pipeline_state.q[7:].at[configs.HUBS].set(self._default_pose[configs.HUBS])
        joint_vel = pipeline_state.qd[6:]
        x = pipeline_state.x

        foot_z = self.wheel_bottoms(pipeline_state)
        contact = foot_z < 1e-3
        ...
        reward = jp.clip(sum(rewards.values()) * self.dt, self._config.reward_floor, 10000.0)
        ...
        state.info["command"] = jp.where(switch, jp.where(command == 0, jax.random.randint(cmd_rng, (), 1, self._num_commands), 0), command)
    ...
    def _get_reward(
        ...
        if cfg.lift_height_mode == 'target':
            lift_height = a_leg_is_up * jp.exp(-jp.square((lifted_height-cfg.target_lift_height)/cfg.lift_height_sigma))
        unload = a_leg_is_up * jp.exp(-self.wheel_loads(pipeline_state)[jp.maximum(lifted_row,0)]/5.)
        centers=pipeline_state.geom_xpos[self._wheels]
        pair_distance=jp.linalg.norm(centers[:,None]-centers[None,:]+jp.eye(4)[:,:,None],axis=-1)
        gap=pair_distance-2*np.hypot(.0505,.01675)
        wheel_separation=jp.sum(jp.triu(jp.square(jp.maximum(.015-gap,0)/.015),k=1))
    ...
    def _get_obs(
        ...
        jpos = (pipeline_state.q[7:] - self._default_pose)[configs.PROXIMAL]
        jpos = jpos + jax.random.uniform(jpos_rng, (8,), minval=-1.0, maxval=1.0) * dr.motor_angle_noise

        last_act = info["last_act"][configs.PROXIMAL]
        last_act = last_act + jax.random.uniform(lact_rng, (8,), minval=-1.0, maxval=1.0) * dr.last_action_noise
