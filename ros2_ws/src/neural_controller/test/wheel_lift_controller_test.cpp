...
#include <fstream>
#include <iostream>
using neural_controller::WheelLiftController;
void require(bool x,const char* m){if(!x)throw std::runtime_error(m);}
class Harness:public WheelLiftController {
 public:
...
int main(int argc,char** argv){rclcpp::init(argc,argv);int result=0;
 ...
 {pluginlib::ClassLoader<controller_interface::ControllerInterface> loader("controller_interface","controller_interface::ControllerInterface");auto plugin=loader.createSharedInstance("neural_controller/WheelLiftController");require(plugin->init("neural_controller_wheel_lift","",520,"",options)==controller_interface::return_type::OK,"installed plugin/real model initializes");}
 Harness c;require(c.init("neural_controller_wheel_lift","",520,"",options)==controller_interface::return_type::OK,"init");require(c.on_configure({})==controller_interface::CallbackReturn::SUCCESS,"configure");
 ...
 for(int i=0;i<300;++i)tick();require(c.fault()==0,"actual model idle updates");c.command(1);tick();require(c.core().leg==0&&c.core().stage==wheel_lift::Core::lift,"actual command subscription buffer drivesFR");
 ...
 c.command(1);tick();require(c.core().stage==wheel_lift::Core::lift,"early align press rejected");
 ...
 reenter();qd[0]=100.;tick();zero();require(c.fault()==4,"infeasible bounded position PD stops");
