#include "neural_controller/triangle_roll_controller.hpp"
#include "pluginlib/class_list_macros.hpp"
#include "neural_controller/triangle_roll_contract.hpp"

namespace neural_controller {
void TriangleRollController::receive_joy(const sensor_msgs::msg::Joy& msg) {
...
bool TriangleRollController::joy_ready() const {
  ...
  try {
    ...
    fault_timer_=get_node()->create_wall_timer(std::chrono::milliseconds(100),[this]() {
      ...
      if(code && code!=reported_fault_)
        RCLCPP_ERROR(get_node()->get_logger(),"TRIANGLE FAULT %d; torque released. Support robot; lifecycle reset required. See TRIANGLE_ROLL_LAB.md.",code);
...
controller_interface::CallbackReturn TriangleRollController::on_activate(const rclcpp_lifecycle::State& s) {
  release_torque();
  try {
    ...
    if(params_.calibration_required) {
      ...
      if(live.calibration_id!=startup_calibration_.calibration_id || j.at("schema_version")!=1 ||
        ...
        throw std::runtime_error("Missing/stale triangle reference; use scripts/inverted_triangle_reference.py --roll");
    ...
    leg_lift_command_subscriber_=get_node()->create_subscription<std_msgs::msg::Int32>(
      "/triangle_roll/command",rclcpp::QoS(1).durability_volatile(),
...
controller_interface::CallbackReturn TriangleRollController::on_deactivate(const rclcpp_lifecycle::State& s) {
  triangle_.stop(inverted_triangle::OPERATOR); consumed_.reset();
  return NeuralController::on_deactivate(s);
}
controller_interface::CallbackReturn TriangleRollController::on_error(const rclcpp_lifecycle::State&) {
...
controller_interface::return_type TriangleRollController::update(const rclcpp::Time& time,const rclcpp::Duration& period) {
  ...
  if(first && dt==0) {
    ...
    triangle_.step(dt,q,qd,triangle_roll::V3(g[0],g[1],g[2]),msg ? msg->data:0,fresh);
...
void TriangleRollController::publish_triangle() {
...
PLUGINLIB_EXPORT_CLASS(neural_controller::TriangleRollController,controller_interface::ControllerInterface)
