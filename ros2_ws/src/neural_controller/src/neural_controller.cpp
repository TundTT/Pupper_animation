...
controller_interface::CallbackReturn NeuralController::on_init() {
  try {
    ...
    if (behavior_ == "locomotion") {
      ...
      if (hybrid_.motion_version != 1 && hybrid_.motion_version != WheelAlignMotion::version)
...
controller_interface::CallbackReturn NeuralController::on_activate(
  ...
  if (behavior_ == "wheel_align_hybrid") {
    ...
    alignment_up_seconds_ = 0.0;
    alignment_timed_out_command_ = -1;
    if (hybrid_.motion_version == WheelAlignMotion::version) {
      ...
      rt_alignment_status_publisher_->msg_.data.resize(13);
    }
    hybrid_elapsed_ = 0.0;
...
controller_interface::return_type NeuralController::update(const rclcpp::Time &time,
  ...
  if (behavior_ == "leg_lift" || behavior_ == "wheel_align_hybrid") {
    ...
    if (leg_lift_command && leg_lift_command->get()) {
      ...
      command_index_ = idx;
      if (behavior_ == "wheel_align_hybrid" && hybrid_.motion_version == WheelAlignMotion::version) {
        if (idx != alignment_timed_out_command_) alignment_timed_out_command_ = -1;
        if (idx == alignment_timed_out_command_) command_index_ = 0;
      }
    }
  }
  ...
  try {
    ...
    if (behavior_ == "wheel_align_hybrid") {
      ...
      if (hybrid_.motion_version == WheelAlignMotion::version) {
        alignment_up_seconds_ = hybrid_.up() ? alignment_up_seconds_ + hybrid_elapsed_ : 0.0;
        // Match the simulation scheduler's 48-second attempt budget. A blocked
        // wheel lowers through the normal trajectory and cannot retry itself.
        if (hybrid_.up() && alignment_up_seconds_ >= 48.0 && command_index_ == hybrid_.command) {
          alignment_timed_out_command_ = command_index_;
          command_index_ = 0;
          RCLCPP_WARN(get_node()->get_logger(), "Alignment timed out after 48s; lowering to stand. Request stand or a different leg before retrying.");
        }
      }
      ...
      if(hybrid_.motion_version==WheelAlignMotion::version) motion_.prepare(hybrid_, hybrid_first_step_ ? WheelAlignMotion::control_dt : hybrid_elapsed_);
  ...
  if (behavior_ == "wheel_align_hybrid") {
    ...
    const bool gate = hybrid_.motion_version==WheelAlignMotion::version ? motion_.ready(hybrid_,hybrid_q_,
    ...
    hybrid_.begin_step(hybrid_q_, gate, hybrid_first_step_ ? (hybrid_.motion_version==WheelAlignMotion::version ? WheelAlignMotion::control_dt : .02) : hybrid_elapsed_);
  ...
  if (behavior_ == "wheel_align_hybrid" && hybrid_.motion_version == WheelAlignMotion::version &&
      rt_alignment_status_publisher_ && rt_alignment_status_publisher_->trylock()) {
    const auto margins = WheelAlignMotion::margins(hybrid_q_, {observation_[3], observation_[4], observation_[5]}, hybrid_.leg());
    auto &data = rt_alignment_status_publisher_->msg_.data;
    int completed = 0;
    for (int k=0;k<4;++k) if(hybrid_.completed[k]) completed |= 1 << k;
    data[0]=hybrid_.phase; data[1]=command_index_; data[2]=hybrid_.active_command;
    data[3]=motion_.progress; data[4]=margins[0]; data[5]=margins[1]; data[6]=margins[2];
    data[7]=hybrid_.was_rotating; data[8]=completed;
    data[9]=WheelAlignHybrid::wrap(hybrid_.target[hybrid_.leg()]-hybrid_q_[3*hybrid_.leg()+2]);
    data[10]=motion_.residual_gain; data[11]=alignment_up_seconds_;
    data[12]=alignment_timed_out_command_;
