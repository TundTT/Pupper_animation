"""Explicit lift/align/lower trial; hardware startup requires STARTUP_UPPER_HOME.md."""
...
def generate_launch_description():
    ...
    candidate=ParameterFile(PathJoinSubstitution([root,'launch','notebook_lift_align_config.yaml']),allow_substs=True)
    ...
    nodes=[Node(package='robot_state_publisher',executable='robot_state_publisher',parameters=[description],output='both'),
           ...
           Node(package='joy_utils',executable='estop_controller',name='joy_util_node',parameters=[base,{
               ...
               'wheel_align_hybrid_command_states':['stand','front_l','front_r','back_r','back_l','rotate'],
               'wheel_align_hybrid_cycle_states':['front_r','rotate','stand','front_l','rotate','stand','back_r','rotate','stand','back_l','rotate','stand'],
           }],output='both')]
