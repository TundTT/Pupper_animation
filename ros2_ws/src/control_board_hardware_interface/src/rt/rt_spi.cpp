...
static bool valid_board[2] = {false, false};
bool spi_feedback_valid() { return valid_board[0] && valid_board[1]; }
...
void spi_send_receive(spi_command_t *command, spi_data_t *data) {
  valid_board[0] = valid_board[1] = false;
  ...
  for (int spi_board = 0; spi_board < 2; spi_board++) {
    ...
    if (rv != static_cast<int>(word_len * 66)) continue;
    ...
    // Reject empty/disconnected frames and checksum failures before copying states.
    const bool empty = g_spine_data.q_abad[0] == 0.f && g_spine_data.q_abad[1] == 0.f &&
        g_spine_data.q_hip[0] == 0.f && g_spine_data.q_hip[1] == 0.f &&
        g_spine_data.q_knee[0] == 0.f && g_spine_data.q_knee[1] == 0.f &&
        g_spine_data.qd_abad[0] == 0.f && g_spine_data.qd_abad[1] == 0.f &&
        g_spine_data.qd_hip[0] == 0.f && g_spine_data.qd_hip[1] == 0.f &&
        g_spine_data.qd_knee[0] == 0.f && g_spine_data.qd_knee[1] == 0.f;
    if (empty || xor_checksum(reinterpret_cast<uint32_t*>(&g_spine_data), 14) !=
        static_cast<uint32_t>(g_spine_data.checksum)) continue;
    bool finite = true;
    for (int i = 0; i < 2; ++i) finite = finite &&
        std::isfinite(g_spine_data.q_abad[i]) && std::isfinite(g_spine_data.q_hip[i]) &&
        std::isfinite(g_spine_data.q_knee[i]) && std::isfinite(g_spine_data.qd_abad[i]) &&
        std::isfinite(g_spine_data.qd_hip[i]) && std::isfinite(g_spine_data.qd_knee[i]);
    if (!finite) continue;
    valid_board[spi_board] = true;
    // copy back to data
