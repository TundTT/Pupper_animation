...
test_dir="$(mktemp -d "${TMPDIR:-/tmp}/walk-policy-test.XXXXXX")"
...
"${CXX:-c++}" -std=c++17 -O2 -DRTNEURAL_USE_EIGEN=1 -DRTNEURAL_DEFAULT_ALIGNMENT=16 \
  ...
  -I"$rt_dir/modules/json" "$controller_dir/test/walk_policy_test.cpp" \
  -o "$test_dir/walk_policy_test"
"$test_dir/walk_policy_test" "$controller_dir/launch/policy_walk_v2.json" \
  "$controller_dir/test/walk_policy_reference.csv"
