...
cmake --build "$repo_dir/ros2_ws/build-combined/neural_controller" --parallel 2 \
  --target joint_pose_controller hub_roll_controller walking_frame_test walking_handoff_test \
...
ctest --test-dir "$repo_dir/ros2_ws/build-combined/neural_controller" \
  -R '^(walking_frame|walking_handoff|walk_policy_contract_and_inference|wheel_policy_contract_and_inference|measured_roll_core|triangle_roll_lifecycle|triangle_roll_stand_lifecycle)$' --output-on-failure
cd "$repo_dir"
