"""Export a trained v5 checkpoint and RTNeural parity fixtures. Never overwrite v1."""
import argparse
import hashlib
import json
...
BLOCKS=[('body_angular_velocity',3),('projected_gravity',3),('effective_command_one_hot',5),
    ...
    ('phase',6),('progress',1),('motion_reference',8),('applied_position',8),('applied_velocity',8),('residual_gain',1)]

def payload_from_params(params,config):
    mean,std=(np.asarray(x,dtype=np.float64) for x in (params[0].mean,params[0].std))
    if mean.shape!=(83,) or std.shape!=(83,) or not np.all(np.isfinite(mean)) or not np.all(np.isfinite(std)) or not np.all(std>0):
        raise ValueError('Invalid observation normalization')
    actor=params[1]['params'];layers=[]
    for i,shape in enumerate(((83,128),(128,128),(128,128),(128,16))):
    ...
    return dict(in_shape=[None,83],layers=layers,behavior='wheel_align_hybrid',motion_contract_version=c.MOTION_VERSION,
        motion_contract_id=c.MOTION_ID,observation_history=1,single_observation_size=83,
        ...
        action_semantics='bounded residual about phase reference; see motion contract v5',
...
def main():
    ...
    x=np.random.default_rng(42).normal(0,.3,(128,83)).astype(np.float32)
