...
def main():
    ...
    p.add_argument('--abduction_guard',type=float,default=.35)
    p.add_argument('--run_tag',default='guard-attempt2')
    ...
    metadata.update(abduction_guard=args.abduction_guard,physical_rotation_margin_m=.005)
    ...
    if args.wandb:
        ...
        run=wandb.init(entity='QuadMorph',project='wheel-leg lift and align triangle base',name=name,config=metadata,id=resume_id,resume='allow',
                       tags=['align-hybrid',args.run_tag,'8-position-4-PD'])
    ...
    _,params,_=train.train(TrainingEnv(abduction_guard=args.abduction_guard),**cfg,network_factory=network_factory(),randomization_fn=randomization(),wrap_env_fn=wrap_for_training,
