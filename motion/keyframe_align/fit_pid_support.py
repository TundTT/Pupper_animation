...
def fit(leg,output,maxfev=280,config=CONFIG):
 ...
 wheel_geoms=[model.geom(f'leg_{name}_3_wheel_collision').id for name in ('front_r','front_l','back_r','back_l')]
 def balance():
  pts=[]
  for k,geom in enumerate(wheel_geoms):
   if k==leg:continue
   axis=d.geom_xmat[geom].reshape(3,3)[:,2];z=np.array([0.,0.,1.]);dot=axis[2];radial=math.sqrt(max(1-dot*dot,1e-12))
   pts.append((d.geom_xpos[geom]-.048*(z-dot*axis)/radial-.01675*np.sign(dot)*axis)[:2])
  pts=np.array(pts);com=d.subtree_com[1,:2];center=pts.mean(axis=0);angles=np.arctan2(*(pts-center)[:,::-1].T);pts=pts[np.argsort(angles)]
  edges=np.roll(pts,-1,axis=0)-pts;v=com-pts
  return float(np.min((edges[:,0]*v[:,1]-edges[:,1]*v[:,0])/np.linalg.norm(edges,axis=1)))
 ...
 def evaluate(x):
  ...
  for n in range(5200):
   ...
   if n>4680:values.append([*o[17:20],tilt,*v[:3],balance()])
   last=o
  c.close()
  if not values:cost=100+peak;tilt=None;support=None
  else:
   v=np.array(values);margins=v[:,:3].min(axis=0);tilt=v[:,3].max();ang=np.linalg.norm(v[:,4:7],axis=1).max();support=v[:,7].min()
   cost=float(30000*np.sum(np.maximum(np.array([.018,.020,.012])-margins,0)**2)+250*max(tilt-.05,0)**2+10*max(ang-.08,0)**2+400*max(peak-.11,0)**2+100000*max(.008-support,0)**2+.008*np.sum((x-base)**2))
  if cost<best[0]:
   best[:]=[cost,x.tolist()];record=dict(leg=leg,cost=cost,pose=x.tolist(),evaluations=count,seconds=time.monotonic()-start,peak_tilt=peak,tail_margins=margins.tolist() if values else None,tail_tilt=tilt,support_margin=support if values else None,source_config=cfg,hardware_validated=False)
   Path(output).write_text(json.dumps(record,indent=2));print(leg,count,round(cost,5),record['tail_margins'],round(peak,3),record['support_margin'],flush=True)
  return cost
 evaluate(base)
