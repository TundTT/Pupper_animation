...
FIELDS = ('entry_seconds', 'shift_seconds', 'lift_seconds', 'land_seconds',
          'recenter_seconds', 'attempt_timeout_seconds', 'wheel_kp', 'wheel_kd',
          'wheel_ki', 'wheel_integral_limit', 'wheel_speed_limit',
...
class Controller:
    def __init__(self, library=None, config=None):
        ...
        values = np.array([self.config[k] for k in FIELDS], dtype=np.float64)
        ...
        self.handle = self.lib.kf_create(values.ctypes.data_as(D), poses.ctypes.data_as(D))
