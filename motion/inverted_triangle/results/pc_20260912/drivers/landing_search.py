...
def evaluate(x):
 ...
 for r,state,previous,base in W:
  ...
  for k in range(n+1040):
   ...
   if k%13:continue
   floor=max(floor,r.unintended_floor_force());tilt=max(tilt,float(np.rad2deg(r.tilt())));bottom=float(r.bottoms()[0])
   if last is not None and bottom<.008:descent=max(descent,(last-bottom)/.025)
   last=bottom
  f=r.contacts_precise();err=abs(r.d.qpos[7+HUB[0]]-base[HUB[0]]);tip=abs(float(r.tip_bottoms()[0]));speed=float(np.abs(r.d.qvel[6:]).max())
...
def main():
 out=Path('runs/inverted_triangle/robust-landing');out.mkdir(exist_ok=False)
 source=Path('runs/inverted_triangle/fixed-plan-v1/03-front_r.json');candidate=json.loads(source.read_text());base=np.array(candidate['landing_pose']);states=[];prefix_steps=0
 for mu in [.5,.65,.8,1.]:
  r=Robot(mu);original=json.loads(Path(f'runs/inverted_triangle/friction-v1/mu-{mu}-seed-1/03-front_r/start_state.json').read_text());home=r.restore(original);previous=home.copy()
  for phase,target,seconds in trajectory(home,np.array(candidate['full_pose']),0,direction=-1,pre_shift_pose=candidate['pre_shift_pose'],landing_pose=base):
   if phase=='land':break
 ...
 config=dict(simulation_only=True,source_hashes=provenance(),environment=versions(),driver_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),parent_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),frictions=[.5,.65,.8,1.],seed=7,prefix_steps=prefix_steps)
 ...
 limits=Robot().m.jnt_range[1:][PROX];x=base[PROX].copy();radius=np.array([.2,.12,.12,.12,.12,.12,.12,.12]);bounds=np.c_[np.maximum(limits[:,0],x-radius),np.minimum(limits[:,1],x+radius)]
 rng=np.random.default_rng(7);pop=np.clip(x+rng.normal(0,.03,(64,8)),bounds[:,0],bounds[:,1]);pop[0]=x;count=0;steps=prefix_steps;best=None;begin=time.monotonic()
 def record(items):
 ...
 with mp.get_context('spawn').Pool(16,initializer=init,initargs=(states,base.tolist())) as pool:
  ...
  differential_evolution(lambda x:0,bounds,init=pop,seed=7,maxiter=20,workers=mapper,updating='deferred',polish=False,tol=1e-5)
 ...
 audit=run(out/'candidate.json',out/'replay',video=True,direction=-1);steps+=audit['environment_steps']
 logger.metrics(steps,{k:v for k,v in audit.items() if isinstance(v,(int,float,bool,str))});logger.video(out/'replay/rollout.mp4',steps,key='trajectory/front_r',caption=f'SIMULATION robust landing candidate {audit["candidate_sha256"]} | search seed 7, replay seed 0 | {audit["status"]} | early termination {audit["early_termination"]}; no trained checkpoint')
