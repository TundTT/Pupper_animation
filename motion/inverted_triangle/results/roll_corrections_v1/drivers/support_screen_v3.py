...
root=Path('runs/roll_to_stand/support-screen-v3')
...
if __name__=='__main__':
 ...
 variants=[dict(force_gain=f,reference_cap=cap,integral_gain=i) for f in [.003,.008] for cap in [.06,.085] for i in [.25,.5]]
 for k,v in enumerate(variants):configs.extend([dict(c,name=f'v{k}-'+c['name'],hold_seconds=18.,balanced_support=v) for c in cases])
