...
class BalancedSupport(AdaptiveSupport):
    def __init__(self,goal,config=None):
        settings=dict(reference_cap=.085,command_cap=.2,force_gain=.008,integral_gain=.5,estimate_time_s=.4,relative_load=True,estimate_mode='vertical')
        settings.update(config or {});super().__init__(goal,settings);self.elapsed=0.
    def update(self,quaternion_wxyz,q,qd,previous_command,dt):
        ...
        for i in range(4):
            ...
            if self.config['estimate_mode']=='effort_norm':load=np.linalg.norm(external)/max(np.linalg.norm(z),.01)
            elif self.config['estimate_mode']=='hybrid':
                blend=np.clip((self.elapsed-self.config.get('norm_delay_s',4.))/4.,0,1)
                load=(1-blend)*load+blend*np.linalg.norm(external)/max(np.linalg.norm(z),.01)
            elif self.config['estimate_mode']!='vertical':raise ValueError('Unknown support estimator')
